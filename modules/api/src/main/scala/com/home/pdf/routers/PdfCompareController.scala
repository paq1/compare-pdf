...
import com.home.pdf.services.TextComparator
    ...
    Action(parse.multipartFormData).async { request =>
      val response = (for {
        ...
        .map {
          case (pdf1, pdf2) =>
            if (!isPdf(pdf1, pdf2)) {
              UnsupportedMediaType(Json.obj("error" -> "les deux fichiers doivent être des pdf"))
            } else {
              // faire notre traitement
              ...
              (for {
                ...
                val (isIdentique, diff) = textComparator.compareLineByLine(text1, text2)
                ...
                Ok(
                  Json.obj(
                    "data" -> Json.obj(
                      ...
                      "attributes" -> Json.obj(
                        "isIdentique" -> isIdentique,
                        "details" -> diff.map(_.toString)
                      )
                    )
                  )
                )
                ...
                .getOrElse(
                  InternalServerError(Json.obj("error" -> "une erreur est survenue"))
                )

            }
        }
        .getOrElse(BadRequest(Json.obj("error" -> "il faut deux pdf (pdf1 et pdf2)")))
...
object PdfCompareController {
  ...
  private def isPdf(file1: FilePartTemporary, file2: FilePartTemporary): Boolean = {
    file1.contentType.contains("application/pdf") && file2.contentType.contains("application/pdf")
  }
}